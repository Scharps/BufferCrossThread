    ...
    where T : unmanaged
{
    ...
    public void WriteChannelData(List<T>[] dataBatch, int length)
    {
    ...
    private void CopyData(int channel, List<T> dataBatch, int length)
    {
        var isOverflow = _cursor + length > Capacity;
        if (!isOverflow) 
        {
            dataBatch.CopyTo(_buffer[channel].AsSpan(_cursor, length));
        }
        else
        {
            var firstCopyLength = Capacity - _cursor;
            var secondCopyLength = length - firstCopyLength;
            dataBatch.CopyTo(0, _buffer[channel], _cursor, firstCopyLength);
            dataBatch.CopyTo(firstCopyLength, _buffer[channel], 0, secondCopyLength);
        }
    }
